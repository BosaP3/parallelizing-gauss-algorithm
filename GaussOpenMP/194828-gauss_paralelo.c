...
int main(int argc, char **argv)
{
    ...
    {
        fprintf(stderr, "Erro: O programa deve ser executado com 2, 4, 8 ou 16 threads.\n");
        return EXIT_FAILURE;
    }
...
void solveLinearSystem(const double *A, const double *b, double *x, int n)
{
    ...
    {
        /*
        Paraleliza o laço que atualiza as linhas abaixo da linha 'i'
        - j é privado por padrão.
        - ratio é privada por ser declarada dentro do escopo do laço.
        - schedule(static) menor overhead para a carga de trabalho balanceada.
        - Acpy e bcpy são compartilhadas por padrão.
        */
        #pragma omp parallel for private(j, count) schedule(static)
        for (j = (i + 1); j < n; j++)
        {
    ...
    /* Back-substitution */
    // loop-carried dependency - cada iteração depende do resultado da anterior
